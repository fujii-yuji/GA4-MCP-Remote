...
| もらうもの | 説明 | 例 |
|---|---|---|
| **MCP サーバーの URL** | Dify / n8n に入力する接続先 | `https://analytics-mcp.example.com/mcp` |
| **Bearer トークン** | 認証用の文字列（パスワードのようなもの） | `dG9rZW4xMjM0NTY3OA==` |
| **GA4 プロパティ ID** | 分析対象のプロパティの数値 | `123456789` |

これら 3 つがあれば、Dify / n8n の設定に進めます。

### エージェント用の AI モデル（Dify / n8n 側）

Dify や n8n でチャットボットを作るには、**MCP とは別に AI モデル（LLM）の API** が必要です。

- Dify なら「モデルプロバイダー」、n8n なら LLM ノードで設定します
- OpenAI（GPT）、Google（Gemini）、Anthropic（Claude）などから選べます
- AI モデルが「どのツールを呼ぶか」「結果をどう説明するか」を判断します
