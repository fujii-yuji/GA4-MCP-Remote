...
from ga4_remote_mcp.ga.tools.utils import (
...
async def list_google_ads_links(property_id: str) -> List[Dict[str, Any]]:
    ...
    Args:
        property_id: The Google Analytics property ID as a string: numeric ID
          (e.g. ``\"123456789\"``) or ``properties/123456789``.
          (Typed as ``str`` only so MCP JSON Schema stays Gemini-compatible;
          ``construct_property_rn`` still accepts numeric strings.)
    """
...
async def get_property_details(property_id: str) -> Dict[str, Any]:
    ...
    Args:
        property_id: Property ID as string (numeric or ``properties/...``).
          See ``list_google_ads_links`` for rationale (Gemini / JSON Schema).
    """
